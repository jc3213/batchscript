...
set viewcomm="Bitmap imageres.dll,70" "JFIF imageres.dll,72" "Jpeg imageres.dll,72" "Png imageres.dll,71" "Tiff imageres.dll,122" "Wdp wmphoto.dll,-400"
for %%a in (%viewcomm%) do (for /f "tokens=1-2 delims= " %%i in (%%a) do (call :miscm1comm %%i %%j))
...
reg add "HKCR\PhotoViewer.FileAssoc.%1\DefaultIcon" /ve /t "REG_SZ" /d "%%SystemRoot%%\System32\%2" /f
exit /b
:miscm1icon
